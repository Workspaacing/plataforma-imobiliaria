...
| `landing-assets`   | **público** para leitura (a página pública usa a URL pública); listar por membros; enviar, substituir e remover por `owner`, `manager` e `assistant` da imobiliária da pasta                                                                                                           | `{organization_id}/landing/{page_id}/{uuid}.{jpg\|jpeg\|png\|webp}` (nome exato; sem SVG, subpastas ou `..`) | jpg, png, webp até 5 MB       |

## Manutenção agendada (pg_cron)

`pg_cron` (grátis no plano Free; migração `scheduled_maintenance`) roda 2 jobs de limpeza. Nenhum dos dois chama HTTP externo nem toca em `audit_events` (retenção LGPD ainda não decidida) ou em dado de negócio (`clients`, `properties`, `leads`, etc.).
...
As 3 funções são `security definer`, `set search_path = ''`, sem `EXECUTE` para `anon`/`authenticated` (só os jobs do `pg_cron` as chamam) e retornam quantas linhas apagaram (`integer`), para ver no histórico do job:
...
Remover um job de vez: `select cron.unschedule('limpeza-nonces-e-tentativas');` (ou `'limpeza-convites-expirados'`). Recriar: reaplique a migração `scheduled_maintenance` (idempotente) ou rode de novo o `select cron.schedule(...)` correspondente.
...
- `0028 anon_security_definer_function_executable`: `get_portal_feed`, `submit_capture_request`, `get_public_organization`, `get_invitation_preview`, `get_public_landing_page`, `submit_landing_lead`, `sync_billing_account`, `get_billing_account_ids`, `list_billing_reminders` e `get_notification_recipients` precisam ser públicas (feed dos portais, formulário de captação, perfil público da imobiliária, prévia do convite, landing page pública, envio de lead, sincronização e lembretes de assinatura, e destinatários de notificação). `submit_capture_request`, `submit_landing_lead`, `sync_billing_account`, `get_billing_account_ids`, `list_billing_reminders` e `get_notification_recipients` só aceitam chamadas com a chave do servidor correspondente (segredos distintos no Vault: `capture_server_key`, `lead_server_key`, `billing_server_key`, `notification_server_key`). [Remediação](https://supabase.com/docs/guides/database/database-linter?lint=0028_anon_security_definer_function_executable)
